...
class Message(BaseModel):
    ...
    timestamp: str = Field(
        description="ISO timestamp for ordering (part of SK)"
    )
    ...
    ttl: int = Field(
        default_factory=lambda: int(
            (datetime.utcnow().timestamp() + DEFAULT_MESSAGE_TTL_SECONDS)
        ),
    ...
    # Internal
    type: str = Field(
        default=ItemType.MESSAGE.value, description="Item type for filtering"
    )
...
class Task(BaseModel):
    ...
    category: str = Field(
        description="Category (work, personal, health, etc) - evolves over time"
    )
    ...
    # Internal
    type: str = Field(
        default=ItemType.TASK.value, description="Item type for filtering"
    )
...
class Todo(BaseModel):
    ...
    # Internal
    type: str = Field(
        default=ItemType.TODO.value, description="Item type for filtering"
    )
...
class Reminder(BaseModel):
    ...
    scheduled_for: str = Field(
        description="When to send reminder (ISO timestamp)"
    )
    ...
    sent_at: Optional[str] = Field(
        default=None, description="When reminder was sent"
    )

    # Internal
    type: str = Field(
        default=ItemType.REMINDER.value, description="Item type for filtering"
    )
